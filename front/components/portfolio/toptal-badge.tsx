...
function ToptalStars() {
  return (
    <svg width="64" viewBox="0 0 60 17" xmlns="http://www.w3.org/2000/svg" aria-hidden focusable="false">
      <path
...
export function ToptalBadge({ className }: { className?: string }) {
  ...
  if (!profile.toptalUrl) return null
  ...
  return (
    <div className={className}>
      ...
      >
        <div
          className="flex w-[200px] flex-col items-center gap-2 bg-[linear-gradient(153deg,#0667ff_18%,#204ecf_40%,#0f256e_80%)] px-4 pb-10 pt-6 text-center text-white shadow-[0_28px_50px_rgba(6,30,96,0.35)]"
          style={{ clipPath: HEXAGON }}
        >
          <ToptalStars />
          <p className="text-[19px] font-bold leading-none">{t.toptal.title}</p>
          <span className="h-px w-[120px] bg-[#25a9ef]" aria-hidden />
          <span className="-mb-1.5 text-base opacity-80">{t.toptal.vettedBy}</span>
          <ToptalWordmark />
          <a
            href={profile.toptalUrl}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center rounded-md bg-[#296bff] px-5 py-1 text-base font-medium text-white underline-offset-2 hover:bg-[#1b5bef] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#0f256e]"
          >
            {t.toptal.cta}
          </a>
        </div>
