...
export function CaseStudyCard({
  ...
  const primaryCaseStudy = project.caseStudies.find((item) => item.featured) ?? project.caseStudies[0]
  const stack = (primaryCaseStudy?.stack.length ? primaryCaseStudy.stack : project.stack).slice(0, 6)
  const categoryLine = [project.category[locale], project.year].filter(Boolean).join(" / ")
  const externalUrl = primaryCaseStudy?.externalUrl ?? project.externalUrl
  const caseStudyCount = project.caseStudies.length
  const caseStudyCountLabel = `${caseStudyCount} ${
    caseStudyCount === 1 ? labels.pages.caseStudy : labels.pages.caseStudies
  }`
  const caseStudyCtaLabel =
    caseStudyCount > 1 ? labels.projects.viewCaseStudies : labels.projects.readCaseStudy
      ...
      <article className="group relative overflow-hidden rounded-lg border border-border/70 bg-card/45 transition-colors hover:border-accent/50">
        <div className="grid min-h-full gap-0 lg:grid-cols-[8rem_minmax(0,1fr)_18rem]">
          ...
          <div className="p-5 md:p-7">
            <div className="font-mono text-[0.7rem] uppercase tracking-[0.18em] text-muted-foreground">{categoryLine}</div>
            ...
            {primaryCaseStudy ? (
              <div className="mt-7 grid gap-5 border-t border-border/60 pt-6 md:grid-cols-2">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-foreground/85">{primaryCaseStudy.problem[locale]}</p>
                </div>
                <div>
                  <div className="font-mono text-[0.68rem] uppercase tracking-[0.18em] text-muted-foreground">
                    {labels.pages.outcome}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-foreground/85">{primaryCaseStudy.outcome[locale]}</p>
                </div>
              </div>
            ) : null}
          </div>
          ...
          <aside className="border-t border-border/60 bg-background/25 p-5 lg:border-l lg:border-t-0 lg:p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-foreground/80">
              {primaryCaseStudy?.systemFocus[locale] ?? project.summary[locale]}
            </p>
